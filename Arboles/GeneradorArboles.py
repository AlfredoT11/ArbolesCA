...
#Última modificación 16/Enero/2021
...
class GeneradorArboles(object):
    ...
    def dibujar_hijos(self, estado, angulo_propio, rango_disponible, centro_estado, radio = 50, is_ciclo = False, siguiente_elemento_ciclo = -1):
        ...
        if hijos[0] in self.relacion_incidencias:
            self.dibujar_hijos(hijos[0], angulo_hijo_1, rango_por_hijo, (self.centro_x+offset_x, self.centro_y+offset_y), radio+50)
        ...
        if num_hijos > 1:
            for i, estado_hijo in enumerate(hijos[1:]):
                ...
                if estado_hijo in self.relacion_incidencias:
                    self.dibujar_hijos(estado_hijo, angulo_hijo_1+(i+1)*rango_por_hijo, rango_por_hijo, (self.centro_x+offset_x, self.centro_y+offset_y), radio+50) 
    ...
    def dibujar_arboles(self):
        ...
        for n_dimension in range(5, 6):
            ...
            if n_dimension > 4:
                ...
                total_datos = 2**(n_dimension*n_dimension)

                siguiente_estado_array = np.zeros((total_datos, ), dtype=np.longlong)
                siguiente_nivel_array = np.zeros((total_datos, ), dtype=int)
                incidencias_array = np.zeros((total_datos, ), dtype=int)

                print("Shapes: {} , {} , {}".format(siguiente_estado_array.shape, siguiente_nivel_array.shape, incidencias_array.shape))

                with open("pruebaDatos.txt", mode = 'r') as file_datos:
                    num_linea = 0
                    for line in file_datos:
                        datos = line.split(',')
                        siguiente_estado_array[num_linea] = int(datos[0])
                        siguiente_nivel_array[num_linea] = int(datos[1])
                        incidencias_array[num_linea] = int(datos[2])
                        num_linea += 1
                
                resultados = [siguiente_estado_array, siguiente_nivel_array, incidencias_array]
...
generador = GeneradorArboles("B2/S7")
generador.dibujar_arboles()

"""super_arreglo = np.zeros(2**25, np.longlong)
super_arreglo_4 = np.zeros(2**25, np.intc)
super_arreglo_8 = np.zeros(2**25, np.longlong)

for i in range(2**25):
    super_arreglo[i] = i
    super_arreglo_4[i] = 10
    super_arreglo_8[i] = i

resultados_listas = []
resultados_listas.append(super_arreglo)
resultados_listas.append(super_arreglo_4)
resultados_listas.append(super_arreglo_8)
